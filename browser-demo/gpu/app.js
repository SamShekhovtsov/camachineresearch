...
const lifeRules = document.getElementById("lifeRules");
...
class WebGpuRunner {
  constructor(device, context, computePipeline, renderPipeline, paramsBuffer) {
    this.label = "WebGPU ping-pong textures";
    this.textures = [];
    this.computeBindGroups = [];
    this.renderBindGroups = [];
    this.currentIndex = 0;
    this.params = new Uint32Array(8);
    this.device = device;
  ...
  static async create(canvasEl) {
    ...
    return new WebGpuRunner(device, context, computePipeline, renderPipeline, paramsBuffer);
  }

  setParams(nextMode, nextBirthMask, nextSurviveMask) {
    this.params[0] = GRID;
    this.params[1] = GRID;
...
class WebGlRunner {
  constructor(canvasEl) {
    this.label = "WebGL2 ping-pong textures";
    this.textures = [];
    this.currentIndex = 0;
    this.params = { mode: 0, birthMask: 0, surviveMask: 0 };
